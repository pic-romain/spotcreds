...
log_filename = f"logs/manual_check_tweets/{datetime.date.today()}.log"
...
if __name__ == "__main__":
    ...
    list_id = input("tweets to check (ids) :").split(",")
    print(list_id)
    artistsRequested = []
    tweets = twitter_api.statuses_lookup(id_=list_id,trim_user=False)
    for t in tweets:
        print(t.text)
        m = re.search(r"playlist\s\S*\s(.*)\s(stp|please|pls)",t.text)
        if bool(m) and t.user.screen_name!="SpotCredits":
            ...
            if len(res)==0:
                unknown_tweet = twitter_api.update_status("@"+t.user.screen_name + f>                unknown_artist = UnknownArtists(
                    tweet_id = t.id,
                    tweet = t._json,
                    artist_name = query
                )
